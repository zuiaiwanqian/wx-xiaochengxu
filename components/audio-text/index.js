  ...
  methods: {
    startAudio (pageType) {
      ...
      if (this.data.storageVoiceId === voiceId) {
        this.setData({ allTime: this.data.totalTime, msPlayTime, playTime }, () => {
        ...
        msPlayTime = 0
        playTime = 0
        this.setData({ msPlayTime, playTime, allTime: this.data.totalTime }, () => {
          if (pageType === 'attached') {
            this.initTime()
          }
          this._play()
    ...
    initTime () {
      let { allTime, endTime, pxTimeRatio, processLeft, processWidth, type, fixedRatio, processLeftRatio } = this.data
      allTime =  Math.floor(allTime)
      this.createSelectorQuery().select('.mark-process-btn').boundingClientRect(res => {
        this.data.processCircleWidth = res.width
      }).exec()
      this.createSelectorQuery().select('.process').boundingClientRect(res => {
      ...
      this.createSelectorQuery().select('.fixed-process').boundingClientRect(res => {
        ...
        processLeftRatio = Number((fixedProcessLeft / processLeft).toFixed(2))
        this.setData({ fixedRatio, processLeftRatio })
      }).exec()
      // 计算置顶音频高度
      this.createSelectorQuery().select('.fixed-main-top').boundingClientRect(res => {
        this.setData({ fixedMainTopHeight: res.height })
      }).exec()
      if (type === 'article') {
        this.createSelectorQuery().selectAll('.article-text').boundingClientRect(res => {
          // 所有文本列表的高度
          this.setData({ articleTextList: res })
        }).exec()
        this.createSelectorQuery().select('.article-text-absolute').boundingClientRect(res => {
          // console.log('一排文字所占的高度', res)
          this.setData({ articleTextOneLineHeight: res.height })
        }).exec()
      }
      if (type === 'card') {
        this.createSelectorQuery().selectAll('.card-audio-text').boundingClientRect(res => {
          // 所有文本列表的高度
          this.setData({ articleTextList: res })
        }).exec()
      }
    ...
    init () {
      ...
      if (type === 'text') {
        ...
        textList = textList.map((item, index) => {
          textList[index + 1] ? item.endTime = textList[index + 1].startTime : item.endTime = totalTime
          return item
        })
    ...
    moveProcessBtn (e) {
      let { processLeft, playTime, pxTimeRatio, scrollTimer, timer, processWidth, processLeftRatio, fixedRatio, fixedAudio, allTime } = this.data
      if (allTime === 0) return
    ...
    tapProcess (e) {
      throttler.limit(() => {
        let { pxTimeRatio, processLeft, playTime, timer, scrollTimer, processLeftRatio, fixedRatio, fixedAudio, allTime } = this.data
        if (allTime === 0) return
        // if(!isPlay) return
        // this.setData({ userControl: true })
        this.data.userControl = true
    ...
    // more的情况滑动
    expandedScroll (type) {
      let { textList, msPlayTime, articleTextList, articleTextOneLineHeight, isScroll, headerHeight, fixedMainTopHeight } = this.data
      if (type === 'system') {
        isScroll = false
      ...
      for (let i = 0; i < textList.length; i++) {
        ...
        if (msPlayTime >= textList[i].startTime && msPlayTime < textList[i].endTime) {
          currentView = i
          break
        }
      }
      ...
      // if (this.data.type === 'article') {
        ...
        if (currentView === 0) {
          ...
          if (this.data.type === 'article') {
            pageScrollTop = articleTextList[currentView].top - fixedMainTopHeight - articleTextOneLineHeight
          } else {
            pageScrollTop = articleTextList[currentView].top - fixedMainTopHeight - 20
          }
        }
        // console.log('进入了', pageScrollTop)
        if (pageScrollTop === this.data.pageScrollTop && type !== 'system') return
    ...
    noExpandedScroll (type) {
      let { textList, playTime, scrollIntoView, isScroll, msPlayTime, showMore, articleTextList, articleTextOneLineHeight } = this.data
      // 改变scrollIntoView
      // 如果用户滑动了页面，就不执行自动滑动
      if (type === 'system') {
        isScroll = false
    ...
    _play (type) {
      ...
      if (this.radio.src !== audioSrc) {
        ...
        this.radio.coverImgUrl = 'https://kf-ui.cdn-go.cn/weapp-image/latest/weapp-sndt/images/logo.png'
